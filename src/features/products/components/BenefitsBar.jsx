...
function BenefitsBar() {
  return (
    <Box component="section" sx={{ bgcolor: "#fff", px: { xs: 2, md: 3.5 }, pt: { xs: 5, md: 7 }, pb: { xs: 4, md: 4 } }}>
      <Box sx={{ display: "grid", gap: { xs: 4, md: 7 }, gridTemplateColumns: { xs: "1fr", md: "repeat(3, 1fr)" }, margin: "0 auto", maxWidth: 830, width: "100%" }}>
        {benefits.map(({ title, description, icon: Icon }) => (
          <Box key={title} sx={{ alignItems: "flex-start", display: "flex", gap: 2.4 }}>
            ...
            <Box>
              <Typography component="h2" sx={{ color: "#111", fontSize: 16, fontWeight: 700, lineHeight: 1.25, mb: 1 }}>{title}</Typography>
              <Typography sx={{ color: "#888", fontSize: 12, lineHeight: 1.55, maxWidth: 190 }}>{description}</Typography>
            </Box>
          </Box>
        ))}
      </Box>
