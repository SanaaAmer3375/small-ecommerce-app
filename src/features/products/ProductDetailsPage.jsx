  ...
  return (
    <Box sx={{ bgcolor: "background.default", py: { xs: 4, md: 8 } }}>
      <Container maxWidth={false} sx={{ maxWidth: 1120, mx: "auto", px: { xs: 2, md: 3.5 } }}>
        <Button startIcon={<ArrowBack />} onClick={() => navigate("/products")} sx={{ color: "text.primary", fontSize: 13, fontWeight: 700, mb: { xs: 3, md: 5 }, textTransform: "none" }}>
          Back to products
        </Button>

        <Box sx={{ display: "grid", gap: { xs: 4, md: 7 }, gridTemplateColumns: { xs: "1fr", md: "1.05fr 0.95fr" } }}>
          <Box>
            <Box sx={{ alignItems: "center", bgcolor: "action.hover", borderRadius: "14px", display: "flex", height: { xs: 360, md: 510 }, justifyContent: "center", overflow: "hidden", p: 3 }}>
            ...
            {images.length > 1 && (
              <Stack direction="row" spacing={1.2} sx={{ mt: 1.5, overflowX: "auto", pb: 0.5 }}>
                {images.map((item, index) => (
                  <IconButton key={item} onClick={() => setActiveImage(index)} sx={{ bgcolor: "action.hover", border: activeImage === index ? "2px solid #2cbd7d" : "2px solid transparent", borderRadius: "8px", height: 68, p: 0.5, width: 68 }}>
          ...
          <Box sx={{ alignSelf: "center" }}>
            ...
            <Typography component="h1" sx={{ color: "text.primary", fontSize: { xs: 31, md: 42 }, fontWeight: 800, letterSpacing: "-1.5px", lineHeight: 1.08, mb: 2 }}>{product.title}</Typography>
            <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 2.4 }}>
              ...
              <Typography sx={{ color: "text.secondary", fontSize: 13 }}>{Number(product.rating || 0).toFixed(1)} rating</Typography>
            </Stack>
            <Typography sx={{ color: "text.secondary", fontSize: 15, lineHeight: 1.75, mb: 3 }}>{product.description}</Typography>
            <Stack direction="row" alignItems="baseline" spacing={1.2} sx={{ mb: 2.5 }}>
              <Typography sx={{ color: "text.primary", fontSize: 30, fontWeight: 800 }}>${discountPrice.toFixed(2)}</Typography>
              {product.discountPercentage > 0 && <Typography sx={{ color: "text.secondary", fontSize: 15, textDecoration: "line-through" }}>${Number(product.price).toFixed(2)}</Typography>}
            ...
            <Stack spacing={1.25} sx={{ mb: 3.2 }}>
              <Typography sx={{ color: "text.secondary", fontSize: 14 }}><Box component="span" sx={{ color: "text.primary", fontWeight: 700 }}>Brand:</Box> {product.brand || "Mina Space"}</Typography>
            ...
            <Stack direction="row" alignItems="center" spacing={1} sx={{ color: "text.secondary", justifyContent: "center", mt: 2.2 }}><LocalShippingOutlined sx={{ fontSize: 19 }} /><Typography sx={{ fontSize: 12 }}>Free standard shipping on orders over $50</Typography></Stack>
          </Box>
        </Box>
      </Container>
    </Box>
